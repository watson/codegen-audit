...
// const { readFileSync, writeFileSync, existsSync, openSync, closeSync, unlinkSync } = require('fs')
...
const shouldAbort = conf.get('abort') === 'true'
...
if (!shouldAbort) {
  ...
  function writeReport (code) {
    ...
    getLockSync(filename, () => {
      ...
      auditLog[scriptName] = [
        ...(
          new Set([...auditLog[scriptName], ...auditor.end()])
        ).values()
      ]
...
function getLockSync (filename, onLock) {
  ...
  while (waitingForLock) {
    try {
      ...
      // const fd = openSync(lockFilename, 'wx')
      // try { closeSync(fd) } catch (er) {}
    } catch (e) {
  ...
  // try { unlinkSync(lockFilename) } catch (er) {}
}
