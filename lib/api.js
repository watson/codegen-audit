...
class CodeGenAuditor extends EventEmitter {
  constructor (opts = {}) {
    super()

    if (CodeGenAuditor._active === true) {
      throw new Error('CodeGenAuditor is already running!')
    }

    CodeGenAuditor._active = true
        ...
        const patchedFn = function () {
          ...
          if (callers.has(frame) === false) {
            ...
            if (opts.errorOnUnknown) {
              const err = new Error(`Illegal call to '${fn.name}' ${frame}`)
              err.frame = frame
              err.fnName = fn.name
              self.emit('error', err)
            }
