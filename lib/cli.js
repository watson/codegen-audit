...
async function parse (filename = defaults.report) {
  const entriesSortedByKey = Object.entries(await loadReports(filename)).sort((a, b) => {
    if (a[0] > b[0]) return 1
    if (a[0] < b[0]) return -1
    return 0
  })
  for (const [key, report] of entriesSortedByKey) {
    if (report.length > 0) {
      console.log(`${key}:`)
      console.log(`  ${report.join(`${EOL}  `)}`)
    }
  }
}
...
async function diff (a, b) {
      ...
      switch (diff.type) {
        ...
        case 'unset': {
          ...
          if (diff.path.length === 1) {
            ...
            console.error(`${verb} invocation in ${diff.path[0]}: ${diff.path[diff.path.length - 1]}`)
          }
          break
        }
        default:
  ...
  function prepareReports (reports) {
    for (const key of Object.keys(reports)) {
      reports[key] = reports[key].reduce(arrToObj, {})
    }
    return reports
...
Options:
  ...
  --report=...   Set custom path to the report file (default:
                 ${defaults.report}).
  --abort        If a call to a code-generation function occurs that is not in
                 the existing report (see --report), the program will exit with
                 an error.
  --no-exit-on-signals
                 Do not exit the process on SIGINT/SIGTERM.
  --verbose      A little chatty.
  --very-verbose Very chatty.
  --persist      Write report to disk as code generation from strings are
                 detected instead of when the process ends, which is the
                 default.
